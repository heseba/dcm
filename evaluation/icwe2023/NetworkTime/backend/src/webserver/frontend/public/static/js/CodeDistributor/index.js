...
export default class CodeDistributor {
  ...
  #registerEventListeners = () => {
    ...
    this.ws.on('updateFragmentList', (data, timestamp) => {
      console.log('Fragmentlist updated');
      const t1 = +new Date();

      const { diffInMs: diff } = getTimeDiff(t1, timestamp);

      console.table([
        {
          updateTimeInMs: diff,
          updateTimeInSec: parseFloat((diff / 1000).toFixed(2)),
        },
      ]);
